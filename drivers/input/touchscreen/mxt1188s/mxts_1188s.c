...
#endif
static int mxt_pinctrl_configure(struct mxt_data *info,
                                                        bool active)
{
...
static int mxt_input_open(struct input_dev *dev)
{
	...
	if (ret == 0) {
        ...
        mxt_pinctrl_configure(data, true);
	ret = mxt_start(data);
	if (ret)
		goto err_open;
...
static void mxt_input_close(struct input_dev *dev)
{
	...
	printk(KERN_ERR "%s\n", __func__);//psb added
        ...
        mxt_pinctrl_configure(data, false);
	mxt_stop(data);
		...
		const struct i2c_device_id *id)
{
	...
	data->pinctrl = devm_pinctrl_get(&client->dev);
        ...
        if (data->pinctrl) {
                error = mxt_pinctrl_configure(data, true);
                if (error)
